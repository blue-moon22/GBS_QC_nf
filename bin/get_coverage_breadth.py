...
def get_coverage_breadth(qc_stats):
...
def write_qc_status(lane_ids, lane_ids_cov_breadth, threshold, headers, output_file):

    headers.insert(0, 'lane_id')
    df = pd.DataFrame(columns=headers, index = [0])

    for ind, lane_id in enumerate(lane_ids):
        cov_breadth = lane_ids_cov_breadth[lane_id]
        df.loc[ind, headers[0]] = lane_id
        df.loc[ind, headers[1]] = cov_breadth

        if cov_breadth is not None:
            if cov_breadth > threshold:
...
def get_arguments():
    parser = argparse.ArgumentParser(description="Get coverage breadth from Pathfind QC stats.")
    ...
    parser.add_argument("-t", "--threshold", required=True, type=int,
                        help="QC PASS/FAIL threshold for coverage breadth i.e. if the coverage breadth is GREATER THAN threshold, then PASS, otherwise FAIL.")
...
def main(args):
    ...
    lane_ids_cov_breadth = get_coverage_breadth(args.qc_stats)

    # Write lane id, number of contigs and PASS/FAIL status in tab file
    write_qc_status(lane_ids, lane_ids_cov_breadth, args.threshold, header_dict["coverage_breadth"], args.output_file)
